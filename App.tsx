...
function AppInner() {
 ...
 useEffect(()=>{const on=()=>{const p=location.hash.replace('#/','') as Page;if(['dashboard','upload','files','duplicates','tags','trash','settings'].includes(p))setPage(p)};window.addEventListener('hashchange',on);return()=>window.removeEventListener('hashchange',on)},[]);
 ...
 return <Layout page={page} setPage={navigate}>{page==='dashboard'&&<Dashboard refreshKey={refresh} onNavigate={navigate}/>}
   {page==='upload'&&<Upload onDone={bump}/>}
   {page==='files'&&<MyFiles refreshKey={refresh} onChange={bump}/>}
   {page==='duplicates'&&<Duplicates refreshKey={refresh} onChange={bump}/>}
